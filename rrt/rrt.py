...
MAP_IMG = './corridor.png'  #Black and white image for a map
MIN_NUM_VERT = 180  #Minimum number of vertex in the graph
...
def rapidlyExploringRandomTree(ax, img, start, goal, seed=None):
  ...
  if goal in points:
    ...
    for i in range(len(path)-1):
      ax.plot([path[i][0], path[i+1][0]], [path[i][1], path[i+1][1]], color='r', linestyle='-', linewidth=2)
      ppl.draw()
